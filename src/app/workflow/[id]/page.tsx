...
function RunDetailDialog({
  ...
  const [run, setRun] = useState<FullRunDetail | null>(null);
  ...
  const statusIcon = (s: string) => {
    if (s === "SUCCESS" || s === "COMPLETED") return <CheckCircle2 size={14} className="text-green-500 shrink-0" />;
    if (s === "FAILED") return <XCircle size={14} className="text-red-500 shrink-0" />;
    if (s === "RUNNING") return <Loader2 size={14} className="animate-spin text-blue-500 shrink-0" />;
    if (s === "PARTIAL") return <AlertCircle size={14} className="text-amber-500 shrink-0" />;
  ...
  const nodeLabel = (nr: NodeRun) =>
    nr.nodeLabel ?? nr.nodeType.replace(/([A-Z])/g, " $1").replace(/^./, (s) => s.toUpperCase()).trim();

  const formatMs = (ms: number | null) =>
    ms == null ? "—" : ms < 1000 ? `${ms}ms` : `${(ms / 1000).toFixed(1)}s`;

  const formatDate = (iso: string) =>
    new Date(iso).toLocaleString("en-US", { month: "short", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: true });

  // Find the final response output
  const responseNodeRun = run?.nodeRuns?.find((nr) => nr.nodeType === "response");
  const finalResponse =
    responseNodeRun?.output?.value ??
    responseNodeRun?.output?.imageUrl ??
      ...
      <DialogContent className="max-w-2xl w-full bg-white rounded-2xl border border-gray-100 shadow-2xl p-0 overflow-hidden max-h-[88vh] flex flex-col">
        <DialogHeader className="px-6 py-4 border-b border-gray-100 shrink-0">
          <div className="flex items-start justify-between">
            <div>
              <DialogTitle className="text-base font-bold text-gray-900">Run Details</DialogTitle>
              {run && (
                <p className="text-xs text-gray-400 mt-0.5 font-mono">{run.id}</p>
              )}
            </div>
            {run && (
              <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${statusChip[run.status] ?? "bg-gray-100 text-gray-600"}`}>
                {run.status}
              </span>
            )}
          </div>
          {run && (
            <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
              ...
              {run.finishedAt && <span>Duration: {formatMs(new Date(run.finishedAt).getTime() - new Date(run.startedAt).getTime())}</span>}
            </div>
          )}
        </DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div className="text-center py-10 text-sm text-gray-400">Failed to load run details.</div>
          ) : (
            <div className="p-6 space-y-4">
              {/* Final Response highlight */}
              {finalResponse && (
                <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-4">
                  <p className="text-xs font-semibold text-indigo-700 mb-2 uppercase tracking-wide">Final Response</p>
                  {typeof finalResponse === "string" && finalResponse.startsWith("http") ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={finalResponse} alt="Output" className="w-full max-h-48 object-contain rounded-lg" />
                  ) : (
                    <p className="text-sm text-indigo-900 whitespace-pre-wrap leading-relaxed">{String(finalResponse)}</p>
                  )}
                </div>
              )}
              ...
              {/* Node run timeline */}
              <div>
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Node Runs ({run.nodeRuns?.length ?? 0})</p>
                <div className="space-y-2">
                  {(run.nodeRuns ?? []).map((nr, idx) => {
                    ...
                    return (
                      <div key={nr.id} className="border border-gray-200 rounded-xl overflow-hidden">
                        {/* Row header */}
                        <button
                          onClick={() => (hasOutput || hasError) && toggleNode(nr.id)}
                          className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors ${hasOutput || hasError ? "hover:bg-gray-50 cursor-pointer" : "cursor-default"
                            }`}
                        >
                          {/* Step number */}
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-900 truncate">{nodeLabel(nr)}</p>
                            <p className="text-xs text-gray-400">{formatMs(nr.durationMs)}</p>
                          </div>
                          <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${statusChip[nr.status] ?? "bg-gray-100 text-gray-600"}`}>
                            {nr.status}
                          </span>
                          {(hasOutput || hasError) && (
                        ...
                        {isExpanded && (
                          <div className="border-t border-gray-100 px-4 pb-4 pt-3 bg-gray-50/50">
                            {hasError && (
                              <div className="mb-3">
                                <p className="text-[10px] font-semibold text-red-600 uppercase mb-1">Error</p>
                                <p className="text-xs text-red-700 bg-red-50 border border-red-200 rounded-lg p-2.5 font-mono whitespace-pre-wrap">{nr.error}</p>
                              </div>
                            )}
                            {hasOutput && (
                              <div>
